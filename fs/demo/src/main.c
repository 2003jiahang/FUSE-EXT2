...
static int demo_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi)
{
    ...
    int temp = ddriver_seek(super.driver_fd, 500 * super.sz_blks, 0);

    /* TODO: 调用ddriver_read读出一个磁盘块到内存，512B */
    char buffer[512]; // 用于存储从磁盘读取的一个块
    if (ddriver_read(super.driver_fd, buffer, super.sz_io) != super.sz_io)
    {
        return -1; // 如果读取失败，则返回错误
    }

    /* TODO: 使用memcpy拷贝上述512B的前sizeof(demo_dentry)字节构建一个demo_dentry结构 */
    struct demo_dentry dentry;
    memcpy(&dentry, buffer, sizeof(dentry)); // 假设demo_dentry结构的大小小于等于512字节

    /* TODO: 填充filename */
    memcpy(filename, dentry.fname, sizeof(filename));
    // filename[sizeof(filename) - 1] = '\0'; // 确保文件名是以NULL结尾
